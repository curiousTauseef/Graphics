...
IBL::IBL(ImageData<float> &latlong, Sphere &sphere) {
	...
	for (int j = 0; j < sphere.diameter; ++j) {
	    ...
	    theta = acos(sphere.normals[index_s + 1]);
	    phi = 2 * (atan2(sphere.normals[index_s], sphere.normals[index_s + 2]) + M_PI/2);
	    ...
	    res[index_s] = dot(v, &r[index_s]) * latlong.data[index_l];
	    res[index_s + 1] = dot(v, &r[index_s]) * latlong.data[index_l + 1];
	    res[index_s + 2] = dot(v, &r[index_s]) * latlong.data[index_l + 2];
	}
    }
